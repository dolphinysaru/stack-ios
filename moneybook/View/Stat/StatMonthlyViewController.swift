//
//  StatMonthlyViewController.swift
//  moneybook
//
...
class StatMonthlyViewController: BaseViewController {
    ...
    private func getStartDate(date: Date) -> Date {
        let month = date.thisMonth
        let year = date.thisYear
        
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd"
        return formatter.date(from: "\(year)-\(month)-01") ?? Date()
    }
    ...
    private func updatePrevious() {
        let date = Calendar.current.date(byAdding: DateComponents(month: -1), to: startDate) ?? Date()
        startDate = getStartDate(date: date)
        endDate = getEndDate(start: startDate)
        updateData()
    }
...
extension StatMonthlyViewController: UITableViewDelegate, UITableViewDataSource {
...
extension StatMonthlyViewController: CalendarItems {
