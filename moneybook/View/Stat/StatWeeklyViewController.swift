//
//  StatWeeklyViewController.swift
//  moneybook
//
...
class StatWeeklyViewController: BaseViewController {
    ...
    override func updateData() {
        ...
        self.itemsOfChart = Item.makeChartData(startDate: startDate, endDate: endDate, items: items)
    ...
    private func getStartDate(date: Date) -> Date {
        ...
        var gapDay = dayNumber - Budget.DayOfTheWeek.monday.rawValue
        if gapDay < 0 {
            gapDay += 7
        }
    ...
    private func getEndDate(start: Date) -> Date {
        ...
        return Calendar.current.date(byAdding: dayComp, to: start) ?? Date()
    }
    
    private func updatePrevious() {
        let date = Calendar.current.date(byAdding: DateComponents(day: -7), to: startDate) ?? Date()
        startDate = getStartDate(date: date)
        endDate = getEndDate(start: startDate)
        updateData()
    }
...
extension StatWeeklyViewController: UITableViewDelegate, UITableViewDataSource {
...
extension StatWeeklyViewController: CalendarItems {
